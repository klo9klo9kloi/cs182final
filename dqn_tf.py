...
class QLearner(object):
    ...
    def __init__(self,
        ...
        # START OF YOUR CODE
        # ----------------------------------------------------------------------
        self.q_t_values = q_func(obs_t_float, self.num_actions, scope='q_func', reuse=False)
        action_mask = tf.one_hot(self.act_t_ph, depth=self.num_actions, on_value=True, off_value=False)
        q_t = tf.boolean_mask(self.q_t_values, action_mask)
        q_target_tp1 = tf.stop_gradient(q_func(obs_tp1_float, self.num_actions, scope='target_q_func', reuse=False))
        if self.double_q:
            q_tp1 = tf.math.argmax(tf.stop_gradient(q_func(obs_tp1_float, self.num_actions, scope='q_func', reuse=True)), axis=1)
            action_mask_tp1 = tf.one_hot(q_tp1, depth=self.num_actions, on_value=True, off_value=False)
            y_t = tf.stop_gradient(self.rew_t_ph + (1-self.done_mask_ph)*gamma*tf.boolean_mask(q_target_tp1, action_mask_tp1))
        else:
            y_t = tf.stop_gradient(self.rew_t_ph + (1-self.done_mask_ph)*gamma*tf.math.reduce_max(q_target_tp1, axis=1))
        self.total_error = tf.math.reduce_mean(huber_loss(y_t- q_t))
        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
    ...
    def step_env(self):
        ...
        explor_prob = self.exploration.value(self.t)

        if not self.model_initialized or np.random.random() < explor_prob:
            action = np.random.randint(self.num_actions)
        else:
            q_vals = self.session.run(self.q_t_values, feed_dict={self.obs_t_ph: [encoded_obs]})
    ...
    def update_model(self):
        ...
        if (self.t > self.learning_starts and \
            ...
            if not self.model_initialized:
                ...
                self.session.run(self.update_target_fn)
            self.session.run([self.train_fn, self.total_error], feed_dict={self.obs_t_ph: obs_batch, self.act_t_ph: act_batch, self.rew_t_ph: rew_batch,
                                                                            self.obs_tp1_ph: next_obs_batch, self.done_mask_ph: done_mask, self.learning_rate: self.optimizer_spec.lr_schedule.value(self.t)})
